...
export class EpisodesListControlComponent extends TemplateGridApplicationComponent implements AfterViewInit {
	...
	onDelete(item: any) {
		this.dataProcessing = true;
		this.service.deleteEpisode(item.id).pipe(take(1)).subscribe({
			next: () => {
				this.dataProcessing = false;
				let snackbarData: any = {
					key: 'delete-item',
