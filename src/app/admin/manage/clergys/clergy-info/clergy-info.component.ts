...
export class ClergyInfoComponent extends SimpleBaseComponent {
	...
	public possisionList$: Observable<any>;
	...
	buildFormGroup() {
		...
		let status = (this.localItem && this.localItem.deActive == 1) ? false : true;
		this.dataItemGroup = this.fb.group({
			...
			possision: [this.localItem ? this.localItem.possision : 'chanh_xu', [Validators.required]],
	...
	getAnniversarys(clergyID: string) {
		...
		// this.service.getAnniversarys(options).pipe(takeUntil(this.unsubscribe)).subscribe({
		// 	next: (res: any) => {
		// 		let anniversarys = [];
	...
	getListClergyPossision() {
		this.possisionList$ = of([]);
		// this.service.getListClergyPossision().pipe(take(1)).subscribe({
		// 	next: (res: any) => {
		// 		if (res && res.value && res.value.length > 0) {
		// 			this.possisionList$ = of(res.value);
		// 		}
		// 	}
		// })
	}
	...
	getSaints() {
		this.saintList$ = of([]);
		this.saintList = [];
		// this.service.getSaints().pipe(take(1)).subscribe({
		// 	next: (res: any) => {
		// 		if (res && res.value && res.value.length > 0) {
	...
	onSaveItem() {
		...
		// 					this.createAnniversaryToTeamMember(res.value._id).pipe(takeUntil(this.unsubscribe)).subscribe({
		// 						next: () => {
		// 							this.dataProcessing = false;
		// 							this.dialogRef.close('OK');
		// 						},
